...
- (void)dealloc {
	[barButtons release];
    [hidedButtons release];
	if (viewToRenderButtons)[viewToRenderButtons release];
	if (viewsToRestoreClipBounds)[viewsToRestoreClipBounds release];
...
- (void)draggingExit:(UIButton *)button withEvent:(UIEvent *)event {
	if (viewsToRestoreClipBounds) {
		...
		[viewsToRestoreClipBounds release];
...
- (void)unregisterForDragEvents:(UIButton*)btn {
	[btn removeTarget:self action:@selector(draggedOut:withEvent:) forControlEvents: UIControlEventTouchDragOutside | UIControlEventTouchDragInside];
}
...
- (void)setViewToRenderButtons:(UIView *)view {
	if (viewToRenderButtons) {
		[viewToRenderButtons release];
	}
	viewToRenderButtons = [view retain];
